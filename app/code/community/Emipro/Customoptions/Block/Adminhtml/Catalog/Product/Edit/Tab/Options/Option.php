    ...
    public function getOptionValues() {
        ...
        if (!$this->_values) {
            ...
            foreach ($optionsArr as $option) {
                ...
                $sql = "select product_id from " . Mage::getConfig()->getTablePrefix() . "catalog_product_option where product_id != " . $feature_id . " and sku = '" . $option->getSku() . "'";
                $result = Mage::getSingleton('core/resource')->getConnection('core_read')->fetchAll($sql);

                if (count($result) == 0 || $this->getProduct()->getId() != $feature_id) {
                    $value['hide_delete'] = '';
                } else {
                    $value['hide_delete'] = "style='display:none'";
                }
